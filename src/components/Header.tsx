  ...
  return (
    <div className={clsx("navbar w-full shadow-md backdrop-blur-md", showBlur ? ' backdrop-blur-md': '')} ref={scrollRef}>
      <div className="navbar-start">
        ...
        <Link className="btn btn-ghost text-xl hidden sm:flex" href="/">Ischia</Link>
      </div>
      ...
      <div className="navbar-end flex items-center gap-6 mr-6">
        ...
        {session?.status === 'unauthenticated' ? <></> : <div onClick={() => signOut()}>{session?.data?.user?.name ?? session?.data?.user?.id} logout</div>}
